...
## Where things are
    ...
    public/assets/img/        Images (create when needed)
...
existing one), then add a line to `NAV` in `public/assets/data.js`. The nav bar on
...
Every page links to `assets/style.css`. If the assets folder is
missing or the files end up loose in `public/`, the site loads as
unstyled text. When uploading to GitHub, drag the FOLDER, not the
files inside it — dragging files flattens the structure.
